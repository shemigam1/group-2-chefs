...
import { recipeFactory } from "../services/factories";
...
export async function createRecipe(req: Request, res: Response) {
  try {
    ...
    const recipe = await recipeFactory().createRecipe(user.id, req.body);
...
export async function updateRecipe(req: Request, res: Response) {
  try {
    ...
    const recipe = await recipeFactory().updateRecipe(
      req.params.recipeId,
      user.id,
      req.body
    );
...
export async function deleteRecipe(req: Request, res: Response) {
  try {
    ...
    const recipe = await recipeFactory().deleteRecipe(
      req.params.recipeId,
      user.id
    );
    return res.status(200).json({
